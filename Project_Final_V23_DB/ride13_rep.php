...
<head>
    <title>Sand Snakes Take Over Ride Report</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <h2 align="center">
        ...
        Sand Snakes Take Over Ride Report
    ...
    <div id="container" style="width: 1000px; height: 600px; margin: 0 auto;">
        <script type="text/javascript">
            <?php
                ...
                for ($x = 1; $x <= 12; $x++) 
                {
                    $nRows = $pdo->query("SELECT COUNT(*) FROM THEMEPARK.RIDES_ON WHERE Ride_date between '2017-$x-01 00:00:00' and '2017-$x-31 23:59:00' and RideOn_ride_id = 13")->fetchColumn(); 
                    array_push($amount0, (int)$nRows);
                }
                for ($x = 1; $x <= 12; $x++) 
                {
                    $nRows = $pdo->query("SELECT COUNT(*) FROM THEMEPARK.RIDES_ON WHERE Ride_date between '2018-$x-01 00:00:00' and '2018-$x-31 23:59:00' and RideOn_ride_id = 13")->fetchColumn(); 
                    array_push($amount1, (int)$nRows);
                }
                for ($x = 1; $x <= 12; $x++) 
                {
                    $nRows = $pdo->query("SELECT COUNT(*) FROM THEMEPARK.RIDES_ON WHERE Ride_date between '2019-$x-01 00:00:00' and '2019-$x-31 23:59:00' and RideOn_ride_id = 13")->fetchColumn(); 
    ...
    <div class="content">
        ...
        <?php
            $pdo = new PDO('mysql:host=themepark.cuoy9m1wadpe.us-east-2.rds.amazonaws.com; dbname=THEMEPARK', 'bryane', 'Bryan12');
            $sth = $pdo->prepare("SELECT Start_date_maintenance, Date_maintenance_closed, Maintenance_type, Maintenance_cost FROM THEMEPARK.RIDE_MAINTENANCE WHERE Ride_ID_maintenance = 13 ORDER BY Start_date_maintenance DESC;");
      $sth->execute();
    ...
    <footer class="w3-container w3-padding-32 w3-center w3-large">
        ...
        <a href="ride13_rep.php"><i class='fa fa-refresh w3-hover-opacity' title="refresh" style="text-align: center; padding-right: 15px"></i></a>
