				...
				else
				{
					...
					if($query_run)
					{
						if(mysqli_num_rows($query_run) > 0)
						{
							...
							@$custid = $row['Cust_bought_ID'];
							@$purchaseid = $row['Purchase_ID'];
							@$cost = $row['Purchase_cost'];
							@$timepurchased = $row['Time_purchased'];
      ...
      if(isset($_POST['viewAll_btn']))
      {
        ...
        echo "<table>";
			...
			else if(isset($_POST['update_btn']))
			{
				...
				{
					$query = "update PURCHASE SET Cust_bought = $custid, Purchase_cost = $cost, Time_purchased = '$timepurchased', Conc_bought_from = $concfrom WHERE Purchase_ID = $purchaseid";
					$query_run = mysqli_query($conn,$query);
