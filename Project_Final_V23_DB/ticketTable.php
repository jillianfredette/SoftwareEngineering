...
<body>
         ...
         <button class="w3-button" id = "home_btn" name = "home" type = "submit" >Home</button>
            <div class="w3-dropdown-hover">
               ...
               <?php if($_SESSION['security_level'] == 3): ?>
                  ...
                  <br>
                  <button class="w3-button" id = "edit_btn" name = "edit_ridesOn_table" type = "submit" style="border: none; text-align: left; width: 100%;">Rides Ons</button>
                  <br>
    ...
    <?php
			if (isset($_POST['fetch_btn']))
			{
				...
				if ($tid == "")
				{
					echo '<script type = "text/javascript">alert("Enter Ticket ID to get ticket details")</script>';
				}
				else
				{
					...
					if($query_run)
					{
						...
						{
							echo '<script type = "text/javascript">alert("Invalid Ticket ID")</script>';
						}
					}
					else
					{
    ...
    <div class="w3-container w3-center">
        <form action="" method="post" accept-charset='UTF-8' style="margin: 0px auto; width: 550px;">
            <fieldset style="width:550px;">
                <label for="eid">Ticket ID:</label>
                <input type="text" placeholder="Ticket ID" name="tid" value="<?php echo $tid; ?>">
                <br><br>
                ...
                <input type="text" placeholder="Customer ID" name="cidnum" value="<?php echo $cidnum; ?>">
                <br><br>
                ...
                <label for="starttime">Ticket Price:</label>
                <input type="text" placeholder="Price" name="price" value="<?php echo $price; ?>">
                <br><br>

                <label for="endtime">Date Purchased (YYYY-MM-DD):</label>
                <input type="text" placeholder="Date Purchased" name="datepurchased" value="<?php echo $datepurchased; ?>">
                <br><br>
                ...
                <div class="w3-bar" style="padding-bottom: 10px">
                    ...
                    <button class="w3-bar-item w3-button" id="btn_clear" name="ticketTable" type="submit">Clear Page</button>
                </div>
            </fieldset>
        </form>
    ...
    <?php
      if(isset($_POST['viewAll_btn']))
      {
        ...
        echo "</tr>";
        $testing = "SELECT * FROM TICKET WHERE Ticket_ID <= 500";
        $query_run = mysqli_query($conn, $testing);
        while($row = mysqli_fetch_array($query_run,MYSQLI_ASSOC)){
          echo "<tr>";
        echo "<td>" .$row['Ticket_ID']. "</td>";
        echo "<td>" .$row['Cust_ID_num']. "</td>";
        ...
        echo "<table>";
			...
			if(isset($_POST['insert_btn']))
			{
				...
				if($tid == "" || $cidnum == "" || $type == "" || $price == "" || $datepurchased == "")
				{
					echo '<script type = "text/javascript">alert("Please insert a value for each field")</script>';
				}
				else
				{
					$query = "insert into TICKET values($cidnum, $tid, '$type', $price, '$datepurchased')";
					$query_run = mysqli_query($conn, $query);
					if($query_run)
					{
						echo '<script type = "text/javascript">alert("Ticket inserted successfully.")</script>';
					}
					else
					{
						echo '<script type = "text/javascript">alert("Error: Ticket not inserted (Does that customer exist?)")</script>';
					}
				}
			...
			else if(isset($_POST['update_btn']))
			{
				...
				{
					$query = "update TICKET SET Cust_ID_num = $cidnum, Ticket_type = '$type', Ticket_Price = $price, Date_Purchased = '$datepurchased' WHERE Ticket_ID = $tid";
					$query_run = mysqli_query($conn,$query);

					if($query_run)
					{
						echo '<script type = "text/javascript">alert("Ticket updated successfully.")</script>';
					}
					else
					{
						echo '<script type = "text/javascript">alert("Error updating ticket (Does that customer exist?)")</script>';
					}
				}
			}
			else if(isset($_POST['delete_btn']))
			{
				if($_POST['tid']=='')
				{
					echo '<script type = "text/javascript">alert("Enter Ticket ID to delete ticket")</script>';
				}
			    else
				{
					$tid = $_POST['tid'];
					$query = "INSERT INTO FLAGGED_FOR_DELETION(Table_name_del, ID_of_entry) VALUES('TICKET',$tid)";
					$query_run = mysqli_query($conn, $query);
					if($query_run)
					{
						echo '<script type = "text/javascript">alert("Ticket flagged for deletion.")</script>';
					}
					else
					{
						echo '<script type = "text/javascript">alert("Error in query.")</script>';
    ...
    <footer class="w3-container w3-padding-32 w3-center w3-large">
        ...
        <a href="ticketTable.php"><i class='fa fa-refresh w3-hover-opacity' title="clear page" style="text-align: center; padding-right: 15px"></i></a>
